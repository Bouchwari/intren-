...
letters — moved out of database.py (Stage 1.5, Prompt 6)."""
import sqlite3
...
_REPORT_COLUMNS = (
    ...
    "ftour_ramadan_expected", "ftour_ramadan_present",
    "shour_expected", "shour_present",
...
def save_daily_reception_record(record: DailyReceptionRecord) -> None:
    ...
    with _connection() as conn:
        conn.execute("""
            INSERT INTO daily_reception_records (date, ftour_qty, ghada_qty, asha_qty,
        ...
        """, (record.date, record.ftour_qty, record.ghada_qty, record.asha_qty,
    ...
    with _connection() as conn:
        ...
        conn.executemany("""
            INSERT INTO order_items
                (letter_id, meal_type, collegial, qualifying, monitors, primary_count)
            VALUES (?,?,?,?,?,?)
        """, [(letter_id, i.meal_type, i.collegial, i.qualifying, i.monitors, i.primary)
              for i in items])
    return letter_id
...
def get_order_items(letter_id: int) -> List[OrderItem]:
    ...
    return [
        OrderItem(
            ...
            monitors=r["monitors"],
            primary=r["primary_count"] if "primary_count" in r.keys() else 0,
        )
        for r in rows
    ]
